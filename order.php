...
while($row_cart=mysqli_fetch_array($run_cart)){
    ...
    while($row_pro=mysqli_fetch_array($run_pro)){
        ...
        (customer_id,product_id,due_amount,invoice_no,qty,size,order_date,order_status) values 
        ...
        // (customer_id,invoice_no,product_id,qty,size,order_status) values
        // ('$customer_id','$invoice_no','$pro_id','$qty','$size','$status');
        // ";
        // $run_pending_order=mysqli_query($con, $insert_pending_order);
        $delete_cart="delete from cart where ip_add='$ip_add'";
        $run_del=mysqli_query($con, $delete_cart);
        echo "<script>alert('Đơn đặt hàng của bạn đã được gửi, xin cảm ơn')</script>";
